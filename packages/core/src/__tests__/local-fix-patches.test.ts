...
import {
  applyLocalFixPatches,
  parseLocalFixPatches,
  type LocalFixPatch,
} from "../utils/local-fix-patches.js";

describe("local-fix patches", () => {
  it("parses patch blocks from the PATCHES section", () => {
    const patches = parseLocalFixPatches([
      "=== PATCHES ===",
      "--- PATCH 1 ---",
      "TARGET_TEXT:",
      "原句一。",
    ...
    expect(patches).toEqual<LocalFixPatch[]>([
  ...
  it("applies a uniquely targeted patch while preserving untouched text", () => {
    ...
    const result = applyLocalFixPatches(original, [
      {
        targetText: "林越没有立刻进去。",
