export interface LocalFixPatch {
  readonly targetText: string;
  readonly replacementText: string;
}

export interface LocalFixPatchApplyResult {
...
const MAX_LOCAL_FIX_TOUCHED_RATIO = 0.25;

export function parseLocalFixPatches(raw: string): LocalFixPatch[] {
  const normalized = raw.includes("=== PATCHES ===")
    ? raw.slice(raw.indexOf("=== PATCHES ===") + "=== PATCHES ===".length)
    : raw;

  const patches: LocalFixPatch[] = [];
...
export function applyLocalFixPatches(
  original: string,
  patches: ReadonlyArray<LocalFixPatch>,
): LocalFixPatchApplyResult {
  if (patches.length === 0) {
    return {
      applied: false,
  ...
  if (original.length > 0 && touchedChars / original.length > MAX_LOCAL_FIX_TOUCHED_RATIO) {
    return {
      applied: false,
      revisedContent: original,
